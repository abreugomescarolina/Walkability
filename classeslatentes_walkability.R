...
poll_walk_complete_w1w4 <- poll_walk_complete %>%
  select( "ID1", matches("w1|w2|w3|w4")) %>%
  rename("ID"="ID1")
...
modelos_trajet <- summarytable(modelo_traj_1c, modelo_traj_2c, modelo_traj_3c, modelo_traj_4c, modelo_traj_5c, which = c("G", "loglik", "conv", "npm", "AIC", "BIC", "SABIC", "entropy", "%class"))
#write.csv(modelos_trajet, "modelos_trajet.csv", row.names = FALSE)

#Melhor modelo (menor BIC) = 3
postprob(modelo_traj_3c)
classes <- as.data.frame(modelo_traj_3c$pprob[,1:2])
